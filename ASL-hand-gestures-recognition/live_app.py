...
class Application:
    ...
    def __init__(self, model_path='cnn8grps_rad1_model.h5', camera_index=0):
        self.vs = cv2.VideoCapture(camera_index)
        if not self.vs.isOpened():
            raise RuntimeError(f'Could not open camera index {camera_index}. Try another camera index, like 1.')
    ...
    def video_loop(self):
        try:
            ...
            if hands:
                ...
                # Keep the crop inside the camera frame so the app does not crash
                # when the hand is close to the edge.
                y1 = max(y - offset, 0)
                ...
                # White background used for the 400x400 hand-landmark drawing.
                # This removes the dependency on a separate white.jpg file.
                white = np.ones((400, 400, 3), dtype=np.uint8) * 255
                # img_final=img_final1=img_final2=0

                hands2, image = hd2.findHands(image, draw=False, flipType=True)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Live Sign Language to Text Application')
    parser.add_argument('--model', default='cnn8grps_rad1_model.h5', help='Path to the trained .h5 Keras model')
    parser.add_argument('--camera', type=int, default=0, help='Camera index. Use 0 for the default webcam.')
    args = parser.parse_args()

    print('Starting Application...')
    app = Application(model_path=args.model, camera_index=args.camera)
    app.root.mainloop()
